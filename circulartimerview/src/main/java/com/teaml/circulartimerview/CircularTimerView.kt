...
class CircularTimerView : View {
    ...
    private fun initPaints(
        ...
        progressColor =
            ta.getColor(R.styleable.CircularTimerView_progressColor, Color.BLUE)
        backgroundColor =
            ta.getColor(R.styleable.CircularTimerView_backgroundColor, Color.GRAY)
        progressBackgroundColor = ta.getColor(
            R.styleable.CircularTimerView_progressBackgroundColor,
            Color.GRAY
        )
        ...
        progressTextColor = ta.getColor(
            R.styleable.CircularTimerView_progressTextColor,
            Color.BLACK
        )
        ...
        val bc =
            String.format("#%06X", 0xFFFFFF and progressBackgroundColor)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (isClockwise) {
            canvas.drawArc(
                mArcBounds,
                startingAngle.toFloat(),
                drawUpto / getMaxValue() * 360,
                false,
                progressBarPaint!!
            )
        } else {
